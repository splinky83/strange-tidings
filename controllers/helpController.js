...
const HandleHelpRequest = ({ res, options }) => {
    ...
    case 'dice':
      res.send({
        ...
        data: {
          ...
          content: `The strangeroll command simulates dice rolls. You can roll any number of ice, with any number of faces.
Each on an individual die will have the same chance of appearing. The randomised results are cryptographically random, so it should be near impossible to predict a result.
...
When using the app, you will be prompted to select a table. The app will then reutrn a random result as if you had rolled on that table.
...
To use the command to a slash, followed by strangetables eg:
/strangetables
...
Dice rolls (in order): 3,1, Result: New Skill

The dice rolls section will show you all the dice rolled (including any dice from subtables) in the order the were rolled, while the Result section will sho you the text from the table.
...
The create faction can only be run by an app adiminstrator.
